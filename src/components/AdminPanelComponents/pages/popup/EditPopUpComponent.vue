        ...
        <div class="input-container">
          ...
          <div class="grid-container">
            <div class="input-group">
              ...
              <select v-model="popupData.campaign_time" disabled>
          ...
          <div v-if="isUpdate" class="input-section">
            <div class="input-group">
              <label class="input-section-header">Status</label>
              <select
                v-model="popupData.live_status"
...
export default {
  ...
  methods: {
    ...
    checkStatus() {
      this.isLoading = true;
      if (this.popupData.live_status == 1) {
        if (this.popupData.live_status == this.temp_live_status) {
          this.updatePopup(this.popupData.live_status, this.popupData.time);
        } else {
          axios
            ...
            .then((response) => {
              if (response.data.success == true) {
                ...
                this.updatePopup(this.popupData.live_status, formattedDate);
              }
            })
            .catch((error) => {
              this.isLoading = false;
    ...
    fetchSinglePopup() {
      ...
      axios
        .get(
          wpApiSettings.root + "custom/v1/get-popup-quiz-data/" + this.itemId,
          {
            headers: {
              "X-WP-Nonce": wpApiSettings.nonce,
            },
        ...
        .then((response) => {
          ...
          this.showPopupData = response.data;
          this.popupData.id = response.data.id;
          this.popupData.image = response.data.image;
          this.popupData.title = response.data.title;
          this.popupData.description = response.data.description;
          this.popupData.popup_time = response.data.popup_time;
          this.popupData.campaign_time = response.data.campaign_time;
          this.popupData.button_link = response.data.button_link;
          this.popupData.button_title = response.data.button_title;
          this.popupData.coupon_code = response.data.coupon_code;
          this.popupData.live_status = response.data.live_status;
          this.temp_live_status = response.data.live_status;
          this.popupData.time = response.data.time;

          if (response.data.button_select == 1) {
            this.showButton = true;
          } else {
            this.showButton = false;
          }

          if (response.data.coupon_select == 1) {
            this.showCoupon = true;
          } else {
            this.showCoupon = false;
          }

          if (this.showPopupData != []) {
            this.isUpdate = true;
          }
          this.showImg = response.data.image;
    ...
    calculateTimeDifference(campaign_time) {
      ...
      const quizTime = new Date(this.popupData.time.replace(" ", "T"));
    ...
    updatePopup(status, time) {
      ...
      if (this.puzzleImage == "") {
        newUploadImage = this.popupData.image;
      } else {
        newUploadImage = this.puzzleImage;
      }
      ...
      superAdminData.append("image", newUploadImage);
      superAdminData.append("title", this.popupData.title);
      superAdminData.append("type", JSON.stringify(superAdminType));

      let updatePopupData = new FormData();
      updatePopupData.append("id", this.popupData.id);
      updatePopupData.append("image", newUploadImage);
      updatePopupData.append("title", this.popupData.title);
      updatePopupData.append("popup_time", this.popupData.popup_time);
      updatePopupData.append("campaign_time", this.popupData.campaign_time);
      updatePopupData.append("description", this.popupData.description);
      updatePopupData.append("button_title", this.popupData.button_title);
      updatePopupData.append("button_link", this.popupData.button_link);
      updatePopupData.append("coupon_code", this.popupData.coupon_code);
      updatePopupData.append("live_status", status);
      updatePopupData.append("time", time);
      updatePopupData.append("button_select", showButtonValue);
      updatePopupData.append("coupon_select", showCouponValue);

      axios
        .post(
          wpApiSettings.root +
            "custom/v1/update-popup-quiz-data/" +
            this.popupData.id,
          updatePopupData,
          {
            headers: {
        ...
        .then((response) => {
          axios
            .post(
              base_url + "update_popup/" + this.userData.user_email,
              superAdminData,
              {
                headers: {
                  "Content-Type": "multipart/form-data",
