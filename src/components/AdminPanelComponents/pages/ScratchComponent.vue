<template>
    ...
    <div class="campaign-header">
      <div class="super-main-header">
        <div class="drag-main-header">
          <div class="main-header-text">Scratch n Win</div>
          <router-link :to="{ name: 'createScratch' }" class="header-button">
            <svg
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in currentRows" :key="index">
              ...
              <td class="action-btn">
                <router-link
                  :to="{ name: 'edit_scratch', params: { id: item.id } }"
                >
                  <div class="edit-btn">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
  ...
  <DeletePop
    ...
    @close="closeModal"
    @confirm="deleteScratch(temp_delete_id)"
  />
  ...
  methods: {
    ...
    groupDelete(selectedItem) {
      if (selectedItem == 1) {
        ...
        axios
          .post(
            wpApiSettings.root + "custom/v1/delete-bulk-scratch-quiz-data",
            data,
            {
              headers: {
                "X-WP-Nonce": wpApiSettings.nonce,
    ...
    fetchAllScratchData() {
      this.isLoading = true;
      axios
        .get(wpApiSettings.root + "custom/v1/get-all-scratch-quiz-data", {
          headers: {
            "X-WP-Nonce": wpApiSettings.nonce,
          },
        })
        .then((response) => {
          ...
          this.totalScratch = response.data.length;
    ...
    deleteScratch(id) {
      ...
      axios
        .delete(
          wpApiSettings.root + "custom/v1/delete-scratch-quiz-data/" + id,
          {
            headers: {
              "X-WP-Nonce": wpApiSettings.nonce,
            },
