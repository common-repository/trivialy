<template>
  <div class="myCampaing-wrap">
    <div class="drag-main-header">
      <div class="main-header-text">Popup</div>
      <router-link :to="{ name: 'createPopUp' }" class="header-button">
        <svg
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(item, index) in currentRows" :key="index">
            ...
            <td class="action-btn">
              <router-link :to="{ name: 'editPopUp', params: { id: item.id } }">
                <div class="edit-btn">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
    ...
    <div class="campaign-footer">
      ...
      <div class="myCampaing-footer">
        ...
        <div class="footer-btn" @click="this.groupDelete(bulkActionSelect)">
          Apply
        </div>
      </div>
    </div>
...
export default {
  created() {
    ...
    if (this.$route.query.showUpdateToast) {
      const showPendingToast = this.$route.query.showUpdateToast;

      // console.log(showPendingToast);
      // Check if showToast is true
      if (showPendingToast == "true") {
        toast.success("Successfully Updated Popup campaign", {
          position: toast.POSITION.BOTTOM_RIGHT,
        });
      }
    }
  ...
  methods: {
    ...
    // checkSubscription(email, id) {
    //   this.isLoading = true;
    //   axios
    //     .get(base_url + "check_activation/" + email)
    //     .then((response) => {
    //       if (response.data.isActive == 1) {
    //         this.openModel(id);
    //       } else if (response.data.isActive == 0) {
    //         toast.error("Please Subscribe To Use This Functionality", {
    //           position: toast.POSITION.BOTTOM_RIGHT,
    //         });
    //       }
    //       this.isLoading = false;
    ...
    // checkSubscriptionGroup(email, ids) {
    //   this.isLoading = true;
    //   axios
    //     .get(base_url + "check_activation/" + email)
    //     .then((response) => {
    ...
    groupDelete(selectedItem) {
      if (selectedItem == 1) {
        ...
        axios
          .post(
            wpApiSettings.root + "custom/v1/bulk-delete-popup-quiz-data",
            data,
            {
              headers: {
                "X-WP-Nonce": wpApiSettings.nonce,
    ...
    deleteSpin(id) {
      axios
        .delete(wpApiSettings.root + "custom/v1/delete-popup-quiz-data/" + id, {
          headers: {
            "X-WP-Nonce": wpApiSettings.nonce,
          },
        })
        .then((res) => {
          ...
          this.isModalVisible = false;
          this.fetchAllPopupData();
          toast.error("Successfully Deleted Popup campaign", {
            position: toast.POSITION.BOTTOM_RIGHT,
          });
        })
        .catch((err) => {
